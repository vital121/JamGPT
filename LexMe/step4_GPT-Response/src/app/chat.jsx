...
// A function that plays an audio from a url and returns a promise that resolves when the audio ends
...
async function imagesGrid({
    ...
    const imagesWidthCoordinates = base64Images.map((src, index) => ({
        src,
    ...
    return await mergeImages(imagesWidthCoordinates, {
        format: "image/jpeg",
        quality,
...
export default function Chat() {
    ...
    async function onSpeech(data) {
        ...
        //adding the image file as a apart of the message to LLM
        await append({
            content: [
            text,
                {
    ...
    //+++triggers a post request to api/chat/route.js to call LLM
    ...
    //To safe Screenshots every few frames
    useEffect(() => {
        const captureFrame = () => {
            if (video.status === "recording" && audio.isRecording) {
                ...
                if (videoNode && canvasNode) {
                    ...
                    canvasNode.width = target.Width;
                    canvasNode.height = target.Width * aspectRatio;
